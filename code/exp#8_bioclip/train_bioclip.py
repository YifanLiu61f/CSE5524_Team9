...
def get_args():
    ...
    p.add_argument('--lr-back',    type=float, default=1e-4)
...
def main():
    ...
    # ---- 4-3 data ----
    ds_full = CsvFathomDataset(args.train_csv, transforms=tf_train_clip)
    val_len = max(1, int(0.1*len(ds_full)))
    ds_train, ds_val = random_split(ds_full, [len(ds_full)-val_len, val_len])
    ds_val.dataset.transforms = tf_val_clip        
    ...
    # only head
    opt = optim.AdamW(model.head.parameters(), lr=args.lr_head, weight_decay=args.wd)
