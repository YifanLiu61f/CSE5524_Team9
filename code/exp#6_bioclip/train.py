...
def main():
    ...
    if args.model_backend == "hf":
        ...
        backbone, preprocess_train, preprocess_val = open_clip.create_model_and_transforms(
            args.model_id      #  pretrained defaults to None → safe
        )
        tf_train, tf_val = preprocess_train, preprocess_val
        img_dim = backbone.visual.output_dim          # save for later
    ...
    elif args.model_backend == "openclip":
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for imgs, labels, _ in loader_train:
            ...
            with autocast(device_type=device.type):
                ...
                logits = out.logits if args.model_backend == "hf" else out
        ...
        # ------- unfreeze at epoch: freeze_epochs ------------------------------------- #
        if epoch == args.freeze_epochs and start_epoch <= args.freeze_epochs:
            for p in backbone_params:
        ...
        with torch.no_grad():
            for imgs, labels, _ in loader_val:
                ...
                t1, t3 = accuracy(logits, labels, topk=(1, 3))
                correct1 += t1.item() * imgs.size(0) / 100
                correct3 += t3.item() * imgs.size(0) / 100
                preds_cat.append(logits.argmax(1).cpu())
        ...
        # ---------------------- LR schedule ------------------------------ #
        if not (epoch == args.freeze_epochs and skip_sched):
            scheduler.step()
