...
def main():
    ...
    # ---------- 2. backbone + val transforms ----------
    backbone, _, preprocess_val = open_clip.create_model_and_transforms(
        "hf-hub:imageomics/bioclip")
    model = BioCLIPClassifier(backbone, len(cat2idx)).to(dev)
    model.eval()

    # ---------- 3. load weights ----------
    state = torch.load(args.checkpoint, map_location=dev)["model"]
    model.load_state_dict(state, strict=True)

    # ---------- 4. data ----------
    ds_test = CsvFathomDataset(args.test_csv, transforms=preprocess_val, test=True)
