...
from utils_metrics import LabelSmoothingCrossEntropy
...
def get_args():
    ...
    p.add_argument('--grad-accum', type=int, default=1,
               help='gradient accumulation steps (simulate bigger batch)')
    return p.parse_args()
...
def main():
    ...
    # ---------- optimiser / sched  ----------
    # param-wise wd 0 for norm / bias -> better for ViT-backbones
    def wd_filter(n):                         # n is parameter name
        return 'bias' in n or 'bn' in n or 'ln' in n or 'norm' in n

    head_pg = [{
        "params": [p for n,p in model.head.named_parameters() if not wd_filter(n)],
        "weight_decay": args.wd
    },{
        "params": [p for n,p in model.head.named_parameters() if wd_filter(n)],
        "weight_decay": 0.
    }]

    opt   = optim.AdamW(head_pg, lr=args.lr_head, betas=(0.9,0.99))
    # cosine **with warm-up** (≈5 % of total iters)
    warm_iters = max(1, int(0.05 * len(train_ld) * args.epochs))
    sched = optim.lr_scheduler.CosineAnnealingLR(opt, T_max=len(train_ld)*args.epochs-warm_iters)

    lr_lambda = lambda it: min(1., it / warm_iters)      # linear warm-up
    warmup = optim.lr_scheduler.LambdaLR(opt, lr_lambda)
    ...
    for ep in range(start_ep, args.epochs+1):
        ...
        model.train()
        tr_loss = 0.
        for step, (imgs, lbls, _) in enumerate(train_ld):
        ...
        if (step + 1) % accum_steps:                     # flush leftovers
            scaler.step(opt); scaler.update()
            warmup.step()
        ...
        sched.step()
        log.info(f'LR {opt.param_groups[0]["lr"]:.2e}')
